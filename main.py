...
try:
    ...
    def about():
        ...
        about_root = Tk()
        about_root.title("Jump Runner")
        about_root.minsize(800, 400)
        about_root.maxsize(800, 400)
        
        # Setting Up ScrollBar
        scrollbar = Scrollbar(about_root)
    ...
    def music():
        ...
        if file_txt[1] == "yes\n":
            resp = messagebox.askyesno(
                title="MUSIC", message="DO YOU WANT TO SWITCH OFF THE MUSIC?"
            )
            if resp == True:
                file_txt[1] = "no\n"
            ...
            resp = messagebox.askyesno(
                title="MUSIC", message="DO YOU WANT TO SWITCH ON THE MUSIC?"
            )
            if resp == True:
                file_txt[1] = "yes\n"
                read_write_file("configuration.txt", "w", file_txt)
    ...
    def sound():
        ...
        if file_txt[2] == "yes\n":
            resp = messagebox.askyesno(
                title="SOUND", message="DO YOU WANT TO SWITCH OFF THE SOUND?"
            )
            if resp == True:
                file_txt[2] = "no\n"
                read_write_file("configuration.txt", "w", file_txt)
            ...
            resp = messagebox.askyesno(
                title="SOUND", message="DO YOU WANT TO SWITCH ON THE SOUND?"
            )
            if resp == True:
                file_txt[2] = "yes\n"
                read_write_file("configuration.txt", "w", file_txt)
    ...
    def highscore():
        ...
        highscore_root = Tk()
        highscore_root.title("Jump Runner")
        highscore_root.minsize(230, 130)
        highscore_root.maxsize(230, 130)
        
        scrollbar = Scrollbar(highscore_root)
        ...
        Button(
            highscore_root,
            text="RESET YOUR HIGHSCORE",
            bg="light sky blue",
            fg="black",
            relief="sunken",
            borderwidth=3,
            command=reset_your_highscore,
        ).place(x=10, y=60)
        ...
        while game_start_stop:
            ...
            if condition:
                ...
                for i in range(0, tiles):
                    screen.blit(background, (i * background.get_width() + move, 0))
